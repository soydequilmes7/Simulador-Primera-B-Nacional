...
from observatorio_ascenso import calcular_observatorio_ascenso
...
def actualizar(n_sims=1000, imprimir=True):
    ...
    # Observatorio del Ascenso, paso 1: guardamos el estado (monte_carlo +
    # tabla_actual) de ANTES de tocar nada -- scraper, tabla, Monte Carlo.
    # Reciclamos el mismo payload que ya persiste /api/estado-nacional
    # (ver data_access.simulation_output()) en vez de inventar un segundo
    # lugar donde guardar "el estado anterior".
    try:
        estado_anterior = data_access.simulation_output("nacional")
    except Exception:
    ...
    # Observatorio del Ascenso, paso 2: comparamos el estado de antes
    # (capturado al principio de esta función, antes del scraper) con el
    # que acaba de salir de la simulación. Es lógica 100% pura (ver
    # observatorio_ascenso.py) -- acá solo la invocamos y persistimos el
    # resultado.
    anteriores_obs = None
    if estado_anterior:
        anteriores_obs = {
            "monte_carlo": estado_anterior.get("monte_carlo"),
            "tabla_actual": estado_anterior.get("tabla_actual"),
        }
    nuevas_obs = {
        "monte_carlo": datos.get("monte_carlo"),
        "tabla_actual": datos.get("tabla_actual"),
    }
    datos["observatorio_ascenso"] = calcular_observatorio_ascenso(anteriores_obs, nuevas_obs)

    # correr_simulacion() ya guardó `datos` en Supabase (guardar_json=True),
    # pero eso fue ANTES de que le agregáramos observatorio_ascenso acá
    # arriba. Volvemos a guardar el payload completo para que el
    # Observatorio quede persistido junto con el resto del estado (no se
    # recalcula la próxima vez que alguien abre/cierra el acordeón, ni al
    # recargar la página -- ver /api/estado-nacional).
    try:
        data_access.save_simulation_output("nacional", "nacional", datos, n_sims)
    except Exception as e:
        if imprimir:
            print(f"  No se pudo persistir el Observatorio del Ascenso: {e}")
