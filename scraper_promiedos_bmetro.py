# -*- coding: utf-8 -*-
"""
scraper_promiedos_bmetro.py

Scraper de Promiedos para la Primera B Metropolitana. Trae el fixture
COMPLETO de la temporada (las 42 fechas de todos contra todos, ida y
vuelta), no solo la fecha actual.

Cómo funciona:
  1) Pide /league/tables_and_fixtures/{LEAGUE_ID} para sacar la lista de
     fechas disponibles (data["games"]["filters"]: "Fecha 1".."Fecha 42",
     cada una con su "key" tipo "5077_100_2_N").
...
status HTTP sea 200 OK. Este es el fix respecto a la versión anterior
de este archivo, que por eso escribía fixture_bmetro.csv y
resultados_bmetro.csv vacíos.
...
columnas que el resto del proyecto:
    ...
    fixture_bmetro.csv:    fecha,jornada,equipo_local,equipo_visitante
    resultados_bmetro.csv: fecha,jornada,equipo_local,equipo_visitante,goles_local,goles_visitante

También se puede importar y usar programáticamente:

    from scraper_promiedos_bmetro import obtener_partidos_jugados_bmetro
    partidos = obtener_partidos_jugados_bmetro()
"""
import csv
import json
import os
...
FIXTURE_CSV = os.path.join(DATOS_DIR, "fixture_bmetro.csv")
RESULTADOS_CSV = os.path.join(DATOS_DIR, "resultados_bmetro.csv")
...
def obtener_fechas_disponibles():
    """Lista de dicts {"name": "Fecha N", "key": "5077_100_2_N"}, sin el
    filtro especial "latest"."""
...
def obtener_partidos_bmetro(verbose=True):
    """Recorre las 42 fechas de la temporada regular y devuelve todos los
...
def obtener_partidos_jugados_bmetro():
    """Igual que obtener_partidos_bmetro(), pero solo los jugados, en el
    formato que espera actualizar_resultados_bmetro.py. Esta API de
    Promiedos no expone goleadores acá (vienen en un campo aparte dentro
    de cada gol de "teams"), así que esas claves quedan vacías por ahora."""
    jugados = [p for p in obtener_partidos_bmetro(verbose=False) if p["jugado"]]
    return [
        {
...
def main():
    ...
    try:
        partidos = obtener_partidos_bmetro()
